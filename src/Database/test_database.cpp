...
TEST_F(FooTest,SaveLoad)
{
    ...
    EXPECT_EQ(base.AddUser(user0),::Database::SUCCESS);
    EXPECT_EQ(base.AddUser(user1),::Database::SUCCESS);
    EXPECT_EQ(base.AddRoom(room),::Database::SUCCESS);
    EXPECT_EQ(base.UserJoinRoom(user0.account,room.id),base.SUCCESS);

    base.Save(sav.c_str());
    EXPECT_EQ(base0.Load(sav.c_str()),::Database::SUCCESS);
    EXPECT_EQ(base0.GetUser(user0.account,user),::Database::SUCCESS);
    EXPECT_EQ(user,user0);
    EXPECT_EQ(base0.GetUser(user1.account,user),::Database::SUCCESS);
    EXPECT_EQ(user,user1);

    EXPECT_EQ(base0.AddUser(user0),::Database::ACCOUNT_EXIST);
    EXPECT_EQ(base0.AddUser(user1),::Database::ACCOUNT_EXIST);
    EXPECT_EQ(base0.AddRoom(room),::Database::ROOM_ID_EXIST);
    EXPECT_EQ(base0.GetRoom(room.id,room0),base0.SUCCESS);
    EXPECT_EQ(room0.members.size(),1);
    EXPECT_EQ(room0.members[0],user0.account);

    EXPECT_EQ(base0.UserJoinRoom(user1.account,room.id),base0.ADMIN_CANNOT_BE_MEMBER);
    EXPECT_EQ(base0.UserJoinRoom(user0.account,room.id),base0.ACCOUNT_EXIST);
